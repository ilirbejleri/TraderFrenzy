...
static int __qran_seed;

void seed_rand(int seed) {__qran_seed = seed;}
...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for(int r = 0; r<height; r++) {
     volatile u16 *dst = videoBuffer +(row+r)*240+col;
...
void drawFullScreenImageDMA(const u16 *image) {
  // TODO: IMPLEMENT
    ...
    DMA[3].cnt = (240 * 160) 
	    | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_NOW | DMA_16 | DMA_ON;
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for (int r = 0; r < height; r++) {
        ...
        DMA[3].cnt = width | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_NOW | DMA_16 | DMA_ON;
    }
}
...
void fillScreenDMA(volatile u16 color) {
  ...
  DMA[3].src = &color;
    DMA[3].dst = videoBuffer;
    DMA[3].cnt = (240 * 160) | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT | DMA_NOW | DMA_16 | DMA_ON;
}
